function [] = FitMLRates_Stringent2(type)
load('NumCpGSites.mat')
%read in the dataset
% load('M.mat')
 sitesnum=num2str(NumCpGSites);
 DatasetName=['Simulated_' sitesnum '_' type];
 ProtocolName='Stringent2';
 Name=['AllDat_' sitesnum '_Sim_Proc_hA_COTCM_Diff_' type '.mat'];
% datafilename=['AllDat_' DatasetName];
load(Name,'AllDat','sites');
...
EnoughReadsLater=0; %number of reads required for later timepoints
...
logmaxlam=1.0;  %log10 maximume allowed fit rate--calculated according to ReadDepth
