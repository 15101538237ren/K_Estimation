...
%ts=[0 0.25 0.5 1.5 2 4 8 16] + 0.5;
ts=[0, 1, 4, 8, 16] + 0.5;
N_Times= length(ts);
...
OUT_FIG_DIR = 'Figures/INFER_METHOD_INCREASE_TIMEPOINTS/INCREASE_AT_8h/';
DATA_SAVE_DIR = 'DATA/INCREASED_TIMEPOINS/';
if ~exist(OUT_FIG_DIR)
    mkdir(OUT_FIG_DIR);
end
if ~exist(DATA_SAVE_DIR)
    mkdir(DATA_SAVE_DIR);
end
for i = 1 : chr_size
    ...
    REP1_DATA_PATH = strcat(DATA_SAVE_DIR, 'REP1.mat'); % Save mat data of rep1 and rep2
    AllDat = All_Reads_rep1;
    save(REP1_DATA_PATH,'AllDat','sites');
    
    REP2_DATA_PATH = strcat(DATA_SAVE_DIR, 'REP2.mat');
    AllDat = All_Reads_rep2;
    save(REP2_DATA_PATH,'AllDat','sites');
    
    REP1_K_RANDOM = strcat(DATA_SAVE_DIR, 'REP1_K_RANDOMT.mat');
    REP2_K_RANDOM = strcat(DATA_SAVE_DIR, 'REP2_K_RANDOMT.mat');
    ...
    FitSites = sites;
    GROUND_TRUTH_K_fp = strcat(DATA_SAVE_DIR, 'GROUND_TRUTH_K.mat');
